...
gss_plot <- function(
  ...
  gp <- ggplot2::ggplot(data = data) +
    ...
    ggplot2::geom_hline(
      data = start_end_temperature,
      ggplot2::aes(yintercept = .data$temperature, linetype = .data$threshold),
      color = "#E8613C"
    ) +
    ggplot2::geom_rect(
      data = gss,
    ...
    ggplot2::geom_segment(
      data = gss_start,
      ggplot2::aes(y = .data$ymin, yend = .data$ymax, x = .data$start_dayte),
      alpha = 1 / 2
    ) +
    ggplot2::geom_segment(
      data = gss_end,
      ggplot2::aes(y = .data$ymin, yend = .data$ymax, x = .data$end_dayte),
      alpha = 1 / 2
    ) +
