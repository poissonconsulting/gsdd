...
test_that("gdd_cf_data shifts by 10", {
  data <- simulated_data
  data$temperature <- data$synthetic
  data <- data[-(1:10),]
  gdd <- gdd_cf_data(data)
...
test_that("gdd_cf_data preserves if shift start date", {
  data <- simulated_data
  data$temperature <- data$synthetic
  data <- data[-(1:10),]
  gdd <- gdd_cf_data(data, start_date = as.Date("2019-02-01"))
  expect_snapshot({
    gdd
  })
})
...
test_that("gdd_cf_data preserves if shift start date", {
  data <- simulated_data
  data$temperature <- data$synthetic
  gdd <- gdd_cf_data(data, start_date = as.Date("2019-03-20"), ignore_truncation = TRUE)
  expect_snapshot({
    gdd
  })
})

test_that("gdd_cf_data one message", {
  data <- simulated_data
  data$temperature <- data$synthetic
  data <- data[-(1:100),]
  expect_message(gdd <- gdd_cf_data(data), "The growing season is truncated at the start of the sequence.")
  expect_snapshot({
    gdd
  })
})
