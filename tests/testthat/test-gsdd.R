...
test_that("vector must be longer than window_width", {
  ...
  expect_chk_error(gsdd(x, window_width = 181))
})

test_that("window_width must be odd", {
  x <- simulated_data$synthetic
  expect_chk_error(gsdd(x, window_width = 6), "`window_width` must be odd.")
})

test_that("gsdd returns NA when missing summer", {
  x <- simulated_data$synthetic
  x[11:360] <- NA_real_
  expect_identical(gsdd(x, msgs = FALSE), NA_real_)
})
...
test_that("gsdd trims missing values", {
  x <- simulated_data$synthetic
  x[c(1,length(x))] <- NA_real_
  expect_equal(gsdd(x), 3898.80557580767)
})

test_that("start temp must be greater than or equal to end temp", {
  x <- simulated_data$synthetic
  expect_chk_error(gsdd(x, end_temp = 40, start_temp = 30))
})

test_that("if max temp in vector is lower than start_temp the function return 0", {
  x <- simulated_data$synthetic
  output <- gsdd(x, start_temp = 50)
  expect_identical(output, 0)
})

test_that("if end_temp is not reached, gsdd calculated to end of vector and message is provided.", {
  x <- simulated_data$synthetic
  expect_message(expect_identical(gsdd(x, end_temp = -40), NA_real_), "The growing season is truncated at the end of the sequence.")
})

test_that("truncated at 100.", {
  x <- simulated_data$synthetic
  x[100] <- NA_real_
  expect_message(expect_identical(gsdd(x), NA_real_), "The growing season is truncated at the start of the sequence.")
})

test_that("truncated at 200.", {
  x <- simulated_data$synthetic
  x[200] <- NA_real_
  expect_message(expect_identical(gsdd(x), NA_real_), "The growing season is truncated at the end of the sequence.")
})

test_that("if end_temp is reached at end of vector x, indicies do not fall off the edge", {
  x <- simulated_data$synthetic
  gsdd <- gsdd(x, end_temp = -4, msgs = FALSE, ignore_truncation = TRUE)
...
test_that("if start_temp is reached at start of vector x, indicies do not fall off the edge", {
  x <- simulated_data$synthetic
  x <- x[163:length(x)]
  gsdd <- gsdd(x, end_temp = 4, msgs = FALSE)
...
test_that("Gets growth period with biggest GSDD even though shorter period.", {
  ...
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, pick = "biggest")
  expect_equal(gsdd, 800)
})

test_that("Gets growth period with biggest GSDD even though shorter period.", {
  x <- c(rep(0, 100), rep(10, 50), rep(0, 50), rep(20, 40), rep(0, 115))
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, pick = "last")
  expect_equal(gsdd, 800)
})

test_that("Gets growth period with biggest GSDD even though shorter period.", {
  x <- c(rep(0, 100), rep(10, 50), rep(0, 50), rep(20, 40), rep(0, 115))
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, pick = "first")
  expect_equal(gsdd, 500)
})

test_that("Gets growth period with smallest GSDD.", {
  x <- c(rep(0, 100), rep(10, 50), rep(0, 50), rep(20, 40), rep(0, 115))
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, pick = "smallest")
  expect_equal(gsdd, 500)
})

test_that("Gets growth period with all GSDD.", {
  x <- c(rep(0, 100), rep(10, 50), rep(0, 50), rep(20, 40), rep(0, 115))
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, pick = "all")
...
test_that("Gets growth period with higher GSDD even though shorter period.", {
  ...
  gsdd <- gsdd(x, window_width = 3, start_temp = 9, end_temp = 9, msgs = FALSE)
  expect_equal(gsdd, NA_real_)
  gsdd <- gsdd(x,
...
test_that("gsdd with two weeks", {
  ...
  expect_equal(gsdd(x), 5.1 * 7 + 3.8 * 6)
})
...
test_that("Gets with two weeks and 3 day window width - great test", {
  ...
  expect_equal(gsdd(x, window_width = 3), 5.1 * 7 + 3.8 * 3)
})
...
test_that("Gets triangle", {
  ...
  expect_equal(gsdd(x), sum(x[9:26]))
})
...
test_that("2 asymetric triangles, first one longer but lower, second should be chosen.", {
  ...
  expect_equal(gsdd(x, pick = "biggest"), sum(x[41:61]))
})
...
test_that("2 asymetric triangles, first one longer but lower, second should be chosen unless longest.", {
  ...
  expect_equal(gsdd(x, pick = "longest"), 193)
})
...
test_that("2 asymetric triangles, second one longer but lower, first one should be chosen.", {
  ...
  expect_equal(gsdd(x, pick = "biggest"), sum(x[3:24]))
})
...
test_that("Right truncated triangle", {
  ...
  expect_equal(gsdd(x, msgs = FALSE), NA_real_)
  expect_equal(gsdd(x, ignore_truncation = "end", msgs = FALSE), sum(x[15:length(x)]))
})
...
test_that("Left truncated triangle", {
  ...
  expect_equal(gsdd(x, msgs = FALSE), NA_real_)
  expect_equal(gsdd(x, ignore_truncation = "start", msgs = FALSE), sum(x[0:25]))
})

test_that("NA if less than 14 values after trimming trailing NAs", {
  x <- c(rep(1,13), rep(NA,100))
  expect_message(expect_identical(gsdd(x),NA_real_), "The length of the longest non-missing sequence in `x` must be at least 14.")
...
test_that("NA if less than 20 values after trimming trailing NAs", {
  ...
  expect_message(expect_identical(gsdd(x, window_width = 11),NA_real_), "The length of the longest non-missing sequence in `x` must be at least 22.")
  x <- c(rep(1,22), rep(NA,100))
  expect_identical(gsdd(x, window_width = 11),0)
})
